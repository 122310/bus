...
from .serializers import BusSerializer,UserSerializer,BookSerializer
...
def findbus(request):
    ...
    if request.method == 'POST':
        ...
        bus_list = Bus.objects.filter(source=source_r, dest=dest_r,route=route_r, date__year=year, date__month=month, date__day=day)
        if bus_list:
        ...
        if bus:
            if bus.rem > int(seats_r):
                ...
                book = Book.objects.create(name=username_r, email=email_r, userid=userid_r, bus_name=name_r,
                                           ...
                                           dest=dest_r,route=route_r, price=price_r, nos=seats_r, date=date_r, time=time_r,
                                           status='BOOKED')
